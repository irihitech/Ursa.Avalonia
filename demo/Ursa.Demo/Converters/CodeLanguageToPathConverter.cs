...
public class CodeLanguageToPathConverter : MarkupValueConverter
{
    private const string AxamlPath =
    ...
    {
        if (value is not CodeLanguage language)
            return AvaloniaProperty.UnsetValue;

        return language switch
        {
            CodeLanguage.Axaml => StreamGeometry.Parse(AxamlPath),
            CodeLanguage.CSharp => StreamGeometry.Parse(CSharpPath),
