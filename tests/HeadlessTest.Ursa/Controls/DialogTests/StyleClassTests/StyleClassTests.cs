...
public class StyleClassTests
{
    ...
    public void StyleClass_Changes_Button_Content()
    {
        ...
        var dialog = ursaWindow.GetVisualDescendants().OfType<StandardDialogControl>().SingleOrDefault();
        Assert.NotNull(dialog);
        Assert.Contains("Custom", dialog.Classes);
        var okButton = dialog.GetVisualDescendants().OfType<Button>().SingleOrDefault(a=>a.Name == StandardDialogControl.PART_OKButton);
        Assert.NotNull(okButton);
        Assert.Equal("CUSTOM", okButton.Content);
        var cancelButton = dialog.GetVisualDescendants().OfType<Button>().SingleOrDefault(a=>a.Name == StandardDialogControl.PART_CancelButton);
